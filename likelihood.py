...
def get_likelihood_fn_pfgm(sde, hutchinson_type='Rademacher',
    ...
    def likelihood_fn(model, data):
        ...
        with torch.no_grad():
            ...
            solution = integrate.solve_ivp(ode_func, (eps, sde.config.sampling.z_max), init, rtol=rtol, atol=atol,
                                           method=method)
            nfe = solution.nfev
            zp = solution.y[:, -1]
            ...
            prior_logp = - torch.log(x_norm ** 2 + sde.config.sampling.z_max ** 2) * (N + 1) / 2. + np.log(
                2 * sde.config.sampling.z_max)
