...
def convert_folder(folder, args, file_writer):
    ...
    for f in files:
        ...
        waveform, sr = librosa.load(f_path, sr=config.sample_rate)
        if use_torch:
            ...
            mel = mel / mel.max()
            # pad with zeros
            pad_len = config.image_size - mel.shape[2]
            if pad_len > 0:
...
def main():
    ...
    print(f"Generating {config.num_mels}x{config.image_size} specs.")
