...
class Downsample(nn.Module):
    def __init__(self, in_ch=None, out_ch=None, with_conv=False):
        ...
        out_ch = out_ch if out_ch else in_ch
        if with_conv:
            self.conv = conv3x3(in_ch, out_ch, stride=2, padding=0)
        self.with_conv = with_conv
        self.out_ch = out_ch
...
class ResnetBlockDDPMpp(nn.Module):
    ...
    def __init__(self, act, in_ch, out_ch=None, zemb_dim=None, conv_shortcut=False, dropout=0.1, skip_rescale=False,
                 init_scale=0.):
        super().__init__()
        out_ch = out_ch if out_ch else in_ch
