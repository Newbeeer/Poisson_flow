...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
    ...
    def pc_sampler(model):
        ...
        with torch.no_grad():
            ...
            if sde.config.training.sde == 'poisson':
                timesteps = torch.linspace(np.log(sde.config.sampling.z_max), np.log(eps), sde.N + 1,
                                           device=device).float()
            else:
            ...
            return inverse_scaler(
                x_mean if denoise else x), sde.N if sde.config.sampling.corrector == 'none' else sde.N * (n_steps + 1)
